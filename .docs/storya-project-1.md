...
**Day 2 (Ngày 2):**
- Generate the UI with **Stitch** using the prompt in §12, iterate, export to Figma. (Tạo UI bằng **Stitch** theo prompt ở §12, tinh chỉnh, xuất sang Figma.)
- Lock the design system: implement `_variables.scss` tokens (color, type, space, motion). (Chốt hệ thiết kế: dựng token trong `_variables.scss`.)
